...
- **Python for both the API and detection.** Python is OpenCV's first-class binding: `opencv-python-headless` ships prebuilt wheels, so setup is `uv sync` with no native toolchain, and masks are NumPy arrays that slice directly. An earlier version of this backend was written in Go with GoCV; it produced the same results, but every machine needed a source-built OpenCV 4 behind cgo and `pkg-config`, and the container needed an x86-only base image. The port returns identical boxes and diagnostics on all four samples. The tradeoff is the GIL: OpenCV releases it inside native calls, which dominate the runtime, so detections run in parallel on a thread pool behind a semaphore, but the short per-candidate Python loop does not. FastAPI adds request validation and generated OpenAPI docs at `/docs`.
